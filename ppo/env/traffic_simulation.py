    ...
    def check_passed(self):
        ...
        if self.direction in ['north', 'south']:
            if (self.direction == 'north' and self.y > exit_dist) or \
               (self.direction == 'south' and self.y < -exit_dist):
                self.passed = True
        else:
            if (self.direction == 'east' and self.x > exit_dist) or \
               (self.direction == 'west' and self.x < -exit_dist):
                self.passed = True
...
class TrafficMetricsCollector:
    ...
    def reset(self):
        ...
        # instantaneous metrics for RL reward
        self.current_queue_sum = 0
        self.last_passed_this_step = 0
        self.current_active_vehicles = 0
    ...
    def update_metrics(self, vehicles_info, current_phase, current_step,
        ...
        self.last_passed_this_step = passed_this_step
        ...
        # instantaneous queue sum this step
        self.current_queue_sum = sum(queue_counts.values())
        ...
        # current active vehicles this step
        self.current_active_vehicles = len(current_waiting_times)
    ...
    def get_metrics(self):
        ...
        # Average waiting time over episode so far
        metrics['average_waiting_time'] = (
            np.mean(self.waiting_times) if self.waiting_times else 0
        )

        # Average queue lengths by direction (history-based)
        metrics['queue_lengths'] = {}
        for direction in ['north', 'south', 'east', 'west']:
            for lane_type in ['straight', 'left']:
                key = f'{direction}_{lane_type}'
                metrics['queue_lengths'][key] = (
                    np.mean(self.queue_lengths[key]) if self.queue_lengths[key] else 0
                )
        ...
        # instantaneous per-step info for RL
        metrics['current_queue_sum'] = self.current_queue_sum
        metrics['passed_this_step'] = self.last_passed_this_step
        metrics['current_vehicles'] = self.current_active_vehicles
...
class TrafficSimulation:
        ...
        for direction in ['north', 'south', 'east', 'west']:
            for lane_type in ['straight', 'left']:
                ...
                if random.random() < prob:
                    ...
                    if len(lane_vehicles) < self.max_lane_vehicles:
    ...
    def get_observation(self):
        """Get current observation state (list of Vehicle objects)"""
