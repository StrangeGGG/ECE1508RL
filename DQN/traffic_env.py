...
class TrafficEngine:
    def __init__(self, spawn_probs=None, min_phase_steps=4, max_phase_steps=40, seed=None):
        if seed is not None:
            random.seed(seed)
            np.random.seed(seed)
        ...
        self.spawn_probabilities = spawn_probs if spawn_probs is not None else default
