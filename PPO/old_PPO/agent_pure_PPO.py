# ================================================================
# agent.py — Pure PPO (no critic) + throughput & wait-time graphs
# ================================================================
import torch
...
class PPOAgent:
    ...
    def select_action(self, state_np: np.ndarray) -> int:
        ...
        state_tensor = (
            torch.from_numpy(norm_state).float().to(self.device).unsqueeze(0)
        )
    ...
    # -------- returns / advantages (no critic) --------
    def compute_returns_and_advantages(self):
        """
        Pure Monte-Carlo returns. We use normalized returns as advantages.
        """
        returns = []
        G = 0.0
    ...
    def update(self):
        ...
        # returns tensor is computed but not used in the loss for pure PPO
...
class SmartTrafficLightControllerPPO:
    ...
    def calculate_reward(self, metrics, old_metrics=None) -> float:
        ...
        if old_metrics is not None and self.phase_duration == 1:
            # penalty for switching phase (to avoid too-frequent switching)
            reward -= 0.1
    ...
    def train(self, episodes=200, max_steps=1000, steps_per_batch=4096):
        print("Starting PPO training with batch updates (pure PPO)...")
        ...
        for ep in range(episodes):
            ...
            print(
                ...
                f"Through={mean_throughput:.2f} | Wait={mean_wait:.2f} | "
                f"Buffer={self.agent.buffer_size()}"
            )
...
def main():
    ...
    print("Training PPO traffic-light controller (batch, pure PPO)...")
    ...
    plt.title("PPO Training Curve (Batch Updates, Pure PPO)")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("ppo_training_batch_pure.png", dpi=300)
    plt.show()

    # throughput graph
    plt.figure(figsize=(10, 4))
    plt.plot(through, label="Throughput")
    plt.xlabel("Episode")
    plt.ylabel("Vehicles Passed")
    plt.title("Throughput Over Episodes (Pure PPO)")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig("ppo_throughput_pure.png", dpi=300)
    plt.show()

    # waiting time graph
    plt.figure(figsize=(10, 4))
    plt.plot(wait, label="Avg Waiting Time")
    plt.xlabel("Episode")
    plt.ylabel("Seconds")
    plt.title("Average Waiting Time Over Episodes (Pure PPO)")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig("ppo_waiting_time_pure.png", dpi=300)
    plt.show()

    controller.agent.save("traffic_ppo_pure_model.pth")
    print("Saved PPO model to 'traffic_ppo_pure_model.pth'")
